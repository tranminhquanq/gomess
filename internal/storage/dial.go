...
func Dial(config *config.GlobalConfiguration) (*Connection, error) {
	...
	if config.DB.Driver != "postgres" {
		logrus.Warn("DEPRECATION NOTICE: only PostgreSQL is supported, will be removed soon")
	} else {
	...
	if config.Metrics.Enabled {
		registerOpenTelemetryDatabaseStats(db) // Register the database connection for metrics
